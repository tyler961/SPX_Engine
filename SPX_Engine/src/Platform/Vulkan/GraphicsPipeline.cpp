...
GraphicsPipeline::GraphicsPipeline(std::string vertFile, std::string fragFile, VkDevice device)
{
	mShaders.push_back(Shader(ShaderType::VERT_SHADER, vertFile, device));
	mShaders.push_back(Shader(ShaderType::FRAG_SHADER, fragFile, device));
}
	...
	VkRenderPass renderPass)
{
	...
	vertexInputInfo.vertexAttributeDescriptionCount = static_cast<SPX_INT>(attributeDescript.size());
