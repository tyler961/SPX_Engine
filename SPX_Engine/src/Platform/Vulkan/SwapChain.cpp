...
{
	SwapChainSupportDetails swapChainSupport = Device::querySwapChainSupport(mPhysicalDevice, mSurface);
	...
	SPX_INT imageCount = swapChainSupport.capabilities.minImageCount + 1;
	...
	QueueFamilyIndices indices = Device::findQueueFamilies(mPhysicalDevice, mSurface);
...
VkExtent2D SwapChain::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& cap, Window* window)
{
	...
	else
	{
		...
		VkExtent2D actualExtent = { static_cast<SPX_INT>(width), static_cast<SPX_INT>(height) };
